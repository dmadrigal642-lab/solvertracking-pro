...
with tab_rastreo:
    ...
    if df_guias.empty:
        ...
        for idx, row in df_guias.iterrows():
            ...
            imp2_val = float(row['impuesto_2']) if pd.notna(row.get('impuesto_2')) else (float(row['impuestos']) if pd.notna(row.get('impuestos')) else 0.0)
            ...
            if c1_val == 0.0 and c2_val == 0.0 and row.get('costo', 0) > 0:
                c1_val = float(row['costo'])
                total_t1 = c1_val
                c_tot = c1_val
